...
class VistaCheesePizza: WKInterfaceController {
    ...
    override func awake(withContext context: Any?) {
        ...
        if(contexto != nil){
            print("tot arrTypeCheese=\(contexto!.arrTypeCheese.count)")
            for i:Int in 0 ..< contexto!.arrTypeCheese.count {
                let item = WKPickerItem()
                item.title = contexto!.arrTypeCheese[i]
    ...
    @IBAction func pickerSelectedItemChanged(_ value: Int) {
        NSLog("List Picker: \(contexto?.arrTypeCheese[value]) selected")
        contexto?.intTypeCheese = value
    }
